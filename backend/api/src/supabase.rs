...
impl SupabaseClient {
    ...
    pub async fn get_today_count(&self, user_id: &str) -> anyhow::Result<SupabaseWillCount> {
        ...
        let request = self.client.get(url.clone()).query(&[
    ...
    pub async fn increment_count(&self, user_id: &str) -> anyhow::Result<SupabaseWillCount> {
        ...
        let current = self
            ...
            .await
            .unwrap_or_else(|_| self.fallback_today(user_id));
        let now = Utc::now().to_rfc3339();
    ...
    fn increment_fallback(&self, user_id: &str) -> SupabaseWillCount {
        ...
        let entry = store
            .entry(key.clone())
            .or_insert_with(|| self.fallback_today(user_id));
        entry.count += 1;
